...
export async function getCurrentUser() {
  if (hasSupabaseAuth) {
    ...
    if (supabase) {
      ...
      if (user) {
        ...
        await getTournamentRepository().ensureProfile(profile);
        return profile;
      }
    }
  }
  ...
  const fallbackId = demoUserId === "organizer" ? DEMO_ORGANIZER_ID : demoUserId === "player" ? DEMO_PLAYER_ID : demoUserId;
